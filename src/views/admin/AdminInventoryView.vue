...
<template>
  <div>
    <div class="page-header mb-6">
      ...
      <button class="btn-primary text-sm gap-1.5">
    ...
    <div
      v-if="
        MOCK_LOTS.some((l) => isNearExpiry(l.expiry_date) || l.quantity === 0)
      "
    ...
    >
      <div class="flex items-center gap-2 mb-3">
        ...
        <span class="font-semibold text-orange-800 text-sm"
          >แจ้งเตือนคลังสินค้า</span
        >
      </div>
      <div class="space-y-1">
        <div
          v-for="lot in MOCK_LOTS.filter(
            (l) => isNearExpiry(l.expiry_date) || l.quantity === 0,
          )"
          :key="lot.id"
        ...
        >
          ...
          <span class="text-orange-500">{{
            lot.quantity === 0 ? "หมด" : `หมดอายุ ${fmtDate(lot.expiry_date)}`
          }}</span>
        </div>
      </div>
    </div>
        ...
        <tbody>
          <tr v-for="p in products" :key="p.id">
            ...
            <td>
              ...
              <span
                v-else-if="p.stock <= p.reorder_level"
            ...
            <td>
              <div class="flex gap-1">
                ...
                <button
                  class="p-1.5 rounded-lg hover:bg-secondary-50 text-secondary-400 hover:text-secondary-700 transition-colors"
                >
                  <Edit class="w-3.5 h-3.5" />
                </button>
                <button
                  @click="showLots = showLots === p.id ? null : p.id"
    ...
    <div v-if="showLots" class="card mt-4">
      ...
      <table class="table">
        ...
        <tbody>
          <tr
            v-for="lot in lotsFor(showLots)"
            :key="lot.id"
            :class="{
              'bg-orange-50':
                isNearExpiry(lot.expiry_date) || lot.quantity === 0,
            }"
          >
            <td class="font-mono text-xs">{{ lot.lot_number }}</td>
            <td
              class="font-semibold"
              :class="lot.quantity === 0 ? 'text-danger' : ''"
            >
              {{ lot.quantity }}
            </td>
            ...
            <td>
              ...
              <span
                v-else-if="isNearExpiry(lot.expiry_date)"
                class="badge badge-yellow"
                >ใกล้หมดอายุ</span
              >
              <span v-else class="badge badge-green">ปกติ</span>
          ...
          <tr v-if="lotsFor(showLots).length === 0">
            <td colspan="6" class="text-center text-secondary-400 py-4">
              ยังไม่มี lot ในระบบ
            </td>
          </tr>
        </tbody>
