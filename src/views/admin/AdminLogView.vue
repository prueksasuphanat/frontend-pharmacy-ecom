...
<template>
  <div>
    <div class="page-header mb-6">
      ...
      <span class="text-sm text-secondary-400">{{ filtered.length }} รายการ</span>
    </div>

    <div class="card mb-4 flex gap-3">
      <div class="relative flex-1">
        <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-secondary-400" />
        <input v-model="search" placeholder="ค้นหา action, admin, ID..." class="input pl-9 text-sm" />
      </div>
    </div>
    ...
    <div class="card p-0 overflow-hidden">
      <table class="table">
        <thead>
          <tr><th>เวลา</th><th>ผู้ดำเนินการ</th><th>Action</th><th>Target</th><th>การเปลี่ยนแปลง</th></tr>
        </thead>
        <tbody>
          <tr v-for="log in paginated" :key="log.id">
            <td class="text-xs text-secondary-400 whitespace-nowrap">{{ fmtDate(log.changed_at) }}</td>
            <td class="text-sm text-secondary-700">{{ log.admin_email }}</td>
            <td><span :class="['badge text-xs', actionColors[log.action] || 'badge-teal']">{{ prettyAction(log.action) }}</span></td>
            ...
            <td class="text-xs max-w-xs">
              <div class="space-y-0.5">
                <div v-if="Object.keys(log.old_value).length > 0" class="text-danger">
                  − {{ JSON.stringify(log.old_value) }}
                </div>
                <div v-if="Object.keys(log.new_value).length > 0" class="text-success">
          ...
          <tr v-if="paginated.length === 0">
            <td colspan="5" class="text-center text-secondary-400 py-8">ไม่พบรายการ</td>
          </tr>
        </tbody>
      </table>
    </div>
