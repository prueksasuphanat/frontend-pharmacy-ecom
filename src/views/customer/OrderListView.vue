...
const orders = computed(() => MOCK_ORDERS.filter(o =>
  auth.isAdmin ? true : o.user_id === auth.currentUser?.id || true // show all for mockup
))
  ...
  <div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else class="space-y-4">
        <RouterLink v-for="order in filtered" :key="order.id" :to="`/orders/${order.id}`"
          class="card-hover block">
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <span class="font-mono font-semibold text-secondary-900 text-sm">{{ order.id }}</span>
                <span :class="['badge', statusConfig[order.status].class]">{{ statusConfig[order.status].label }}</span>
                <span v-if="order.prescription_status === 'pending'" class="badge badge-yellow">ใบสั่งยารอรีวิว</span>
              </div>
              <p class="text-xs text-secondary-400">{{ fmtDate(order.created_at) }} · {{ order.items.length }} รายการ</p>
              <div class="flex gap-2 mt-2 flex-wrap">
                <div v-for="item in order.items.slice(0,3)" :key="item.id" class="flex items-center gap-1.5">
                  <img :src="item.product_image" :alt="item.product_name" class="w-8 h-8 rounded object-cover" />
                  <span class="text-xs text-secondary-500 hidden sm:block">{{ item.product_name }}</span>
                </div>
                <span v-if="order.items.length > 3" class="text-xs text-secondary-400 self-center">+{{ order.items.length - 3 }} รายการ</span>
              </div>
            </div>
