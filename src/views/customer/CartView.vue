...
import { Trash2, Plus, Minus, Heart, ShoppingBag, ArrowRight } from 'lucide-vue-next'
        ...
        <div class="lg:col-span-2 space-y-3">
          <div v-for="item in cart.items" :key="item.product_id"
            ...
            <div class="flex-1 min-w-0">
              <p class="font-semibold text-secondary-900 leading-snug">{{ item.product_name }}</p>
              <p class="text-xs text-secondary-400 mt-0.5">{{ item.sku }} · {{ item.unit }}</p>
              <p class="text-warning text-xs mt-1 font-medium" v-if="item.requires_prescription">
              ...
              <div class="flex items-center justify-between mt-3">
                ...
                <p class="font-bold text-primary-700">฿{{ fmt(item.unit_price * item.quantity) }}</p>
              </div>
            </div>
            <div class="flex flex-col gap-2 shrink-0">
              <button @click="cart.moveToWishlist(item.product_id)"
                class="p-1.5 rounded-lg text-secondary-300 hover:text-primary-500 hover:bg-primary-50 transition-colors" title="ย้ายไป Wishlist">
                <Heart class="w-4 h-4" />
              </button>
              <button @click="cart.removeItem(item.product_id)"
        ...
        <div class="space-y-4">
          <div class="card">
            ...
            <div v-if="cart.shippingFee > 0" class="mt-3 p-3 bg-primary-50 rounded-lg">
              <p class="text-xs text-primary-700">🚚 สั่งอีก ฿{{ fmt(1000 - cart.subtotal) }} รับสิทธิ์จัดส่งฟรี</p>
            </div>

            <RouterLink to="/checkout" class="btn-primary w-full mt-4 flex items-center justify-center gap-2">
              ดำเนินการชำระเงิน <ArrowRight class="w-4 h-4" />
            </RouterLink>
            <RouterLink to="/products" class="btn-secondary w-full mt-2 text-center text-sm">เลือกสินค้าเพิ่ม</RouterLink>
          </div>

          <div v-if="cart.hasPrescriptionItem" class="card bg-yellow-50 border border-yellow-200">
