...
const isSpecialPrice = computed(() => selectedUnit.value?.is_special_price ?? false);
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="space-y-5">
          <div>
            ...
            <p v-if="product.generic_name" class="text-secondary-500 text-sm mt-1">
          ...
          <div v-if="product.units && product.units.length > 1" class="card">
            <h3 class="font-semibold text-secondary-900 mb-3 text-sm">เลือกหน่วย</h3>
          ...
          <div class="card bg-primary-50 border-primary-100">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p v-if="displayPrice > 0" class="text-3xl font-bold text-primary-700">
                  ฿{{ formatPrice(displayPrice) }}
                </p>
                <p v-else class="text-lg font-medium text-secondary-500">ติดต่อสอบถาม</p>
                <p v-if="isSpecialPrice" class="text-xs text-primary-600 mt-0.5 font-medium">
                  ✨ ราคาพิเศษสำหรับคุณ
                </p>
                <p v-if="selectedUnit" class="text-xs text-secondary-400 mt-0.5">
              ...
              <div class="text-right">
                ...
                >
                  <CheckCircle class="w-4 h-4" /> มีสินค้า ({{ product.quantity }})
                </div>
                <div
                  v-else
          ...
          <div class="card">
            ...
            <div class="space-y-2 text-sm">
              <div
                ...
                class="flex gap-3"
              >
                <span class="text-secondary-400 w-28 shrink-0">{{ row.label }}</span>
          ...
          <!-- Description -->
          <p v-if="product.using" class="text-secondary-600 text-sm leading-relaxed">
            <span class="font-medium text-secondary-800">วิธีใช้:</span> {{ product.using }}
          </p>
          <p v-if="product.warning" class="text-orange-600 text-sm leading-relaxed">
            <span class="font-medium">⚠️ คำเตือน:</span> {{ product.warning }}
          </p>

          <!-- Add to cart -->
          <div v-if="auth.isLoggedIn" class="space-y-3">
            <div v-if="inStock && displayPrice > 0" class="flex items-center gap-3">
              <div
            ...
            <p v-else-if="!inStock" class="text-sm text-secondary-400 text-center py-2">
              สินค้าหมดชั่วคราว
            </p>
          </div>
          <div v-else class="card bg-secondary-50 text-center">
            <p class="text-sm text-secondary-500 mb-2">เข้าสู่ระบบเพื่อสั่งซื้อ</p>
            <RouterLink to="/auth/login" class="btn-primary text-sm">เข้าสู่ระบบ</RouterLink>
          </div>
        </div>
      </div>
    </div>
