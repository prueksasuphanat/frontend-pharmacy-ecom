...
import { User, Package, MapPin, Lock, Heart, ChevronRight } from 'lucide-vue-next'
  ...
  <div>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div class="card mb-4 flex items-center gap-4">
        ...
        <div class="flex-1">
          <p class="font-bold text-secondary-900">{{ auth.currentUser?.full_name || 'ผู้ใช้งาน' }}</p>
          <p class="text-sm text-secondary-500">{{ auth.currentUser?.email }}</p>
      ...
      <div class="card space-y-1 p-2">
        <RouterLink v-for="item in [
          ...
          <span class="text-sm text-secondary-700 flex-1">{{ item.label }}</span>
          <ChevronRight class="w-4 h-4 text-secondary-300" />
        </RouterLink>
      </div>
      <p class="text-xs text-secondary-300 text-center mt-4">TODO: เชื่อม PATCH /profile</p>
    </div>
  </div>
</template>
