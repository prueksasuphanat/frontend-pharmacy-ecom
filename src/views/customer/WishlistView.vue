...
<template>
  <div>
    ...
    <div class="max-w-5xl mx-auto px-4 py-8">
      <h1 class="page-title mb-6">รายการที่บันทึกไว้ <Heart class="inline w-5 h-5 text-red-500 fill-red-500" /></h1>
      <div v-if="wishlist.length === 0" class="text-center py-20">
        ...
        <RouterLink to="/products" class="btn-primary mt-4">เลือกสินค้า</RouterLink>
      </div>
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        <div v-for="p in wishlist" :key="p.id" class="card p-0 overflow-hidden">
          <img :src="p.image_url" :alt="p.name" class="w-full h-36 object-cover" />
          <div class="p-3 space-y-2">
            <p class="text-sm font-medium text-secondary-900 line-clamp-2">{{ p.name }}</p>
            <p class="text-primary-700 font-bold text-sm">฿{{ fmt(getPrice(p.id)) }}</p>
            <div class="flex gap-2">
              <button @click="cart.addToCart(p.id)" class="btn-primary flex-1 text-xs py-1.5 gap-1">
                <ShoppingCart class="w-3 h-3" /> ใส่ตะกร้า
              </button>
              <button @click="remove(p.id)" class="p-1.5 rounded-lg border border-secondary-200 text-secondary-400 hover:text-danger hover:border-red-200 transition-colors">
