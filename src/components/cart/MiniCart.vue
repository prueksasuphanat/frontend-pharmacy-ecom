...
import { ShoppingCart, X, Trash2, Plus, Minus, ShoppingBag } from 'lucide-vue-next'
...
<template>
  ...
  <button @click="isOpen = !isOpen"
    ...
    <span v-if="cart.totalItems > 0"
      class="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-primary-600 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
  ...
  <Transition name="fade">
    <div v-if="isOpen" class="fixed inset-0 bg-black/30 z-40" @click="isOpen = false" />
  </Transition>
  ...
  <Transition name="slide-cart">
    <div v-if="isOpen"
      ...
      <div class="flex items-center justify-between px-5 py-4 border-b">
        ...
        <button @click="isOpen = false" class="p-2 rounded-lg hover:bg-secondary-50">
          <X class="w-5 h-5" />
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        <div v-if="cart.items.length === 0" class="text-center py-12">
          ...
          <RouterLink to="/products" @click="isOpen=false" class="btn-primary mt-4 text-sm">
            ดูสินค้า
          </RouterLink>
        </div>
        ...
        <div v-for="item in cart.items" :key="item.product_id"
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-secondary-800 line-clamp-2 leading-snug">{{ item.product_name }}</p>
            <p class="text-xs text-secondary-400 mt-0.5">{{ item.unit }}</p>
            <p class="text-primary-600 font-semibold text-sm mt-1">฿{{ formatPrice(item.unit_price) }}</p>
          </div>
          <div class="flex flex-col items-end gap-2 shrink-0">
            <button @click="cart.removeItem(item.product_id)" class="p-1 text-secondary-300 hover:text-danger transition-colors">
      ...
      <div v-if="cart.items.length > 0" class="border-t px-5 py-4 space-y-3">
        <div class="space-y-1.5 text-sm">
          ...
          <div class="flex justify-between font-bold text-secondary-900 text-base border-t pt-1.5">
            <span>ยอดรวมทั้งหมด</span><span class="text-primary-600">฿{{ formatPrice(cart.total) }}</span>
          </div>
        </div>
        <p v-if="cart.shippingFee > 0" class="text-xs text-secondary-400">
          สั่งซื้อครบ ฿1,000 รับสิทธิ์จัดส่งฟรี
        </p>
        <RouterLink to="/checkout" @click="isOpen=false" class="btn-primary w-full text-center">
          ดำเนินการชำระเงิน
        </RouterLink>
        <RouterLink to="/cart" @click="isOpen=false" class="btn-secondary w-full text-center text-sm">
          ดูตะกร้าทั้งหมด
        </RouterLink>
      </div>
    </div>
